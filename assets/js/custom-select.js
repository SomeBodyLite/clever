class ItcCustomSelect {
  ...
  static template(params) {
    ...
    let selectedIndex = -1;
    let selectedValue = '';
    let selectedContent = 'Choose from list';
    ...
    const toggleClass = isSelected ? ' selected' : '';
    return `<button type="button" class="itc-select__toggle${toggleClass}" name="${name}"
  ...
  _updateOption(el) {
    ...
    this._elToggle.classList.add('selected');
  ...
  _reset() {
    ...
    this._elToggle.classList.remove('selected');
    this._el.dispatchEvent(new CustomEvent('itc.select.change'));
